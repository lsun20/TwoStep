...
putexcel set "monte_carlo_2sls_power_weak_het.xlsx", replace
matrix r = J(1,8,.)
...
forval j = 1/500{
	...
	generate X2_weak_het = Z1*0.1 + Z2*0.2 + Z3*0.3 + Z4*0.4 + Z5*0.5 + u1*het 
	generate X1_weak_het = Z1*0.25 + Z2*0.25 + Z3*0.25 + Z4*0.25 + Z5*0.25 + u2*het
	...
	forval t = -25/25 { // calculate test statistics for null=0 over 51 true values
		...
		quietly {
			...
			local tl = -`norm'*6
			local tu = `norm'*6
			cap drop Y2_weak_het
			generate Y2_weak_het = X1_weak_het*`theta' + X2_weak_het*0 + epsilon*het
			
			twostepweakiv 2sls Y2_weak_het (X1_weak_het X2_weak_het = Z1 Z2 Z3 Z4 Z5) if _n <= 500,noconstant gridmin(0 `tl') gridmax(0 `tu') gridpoints(1 101) citestlist(lc_2sls k_2sls)
			...
			twostepweakiv 2sls Y2_weak_het (X1_weak_het X2_weak_het = Z1 Z2 Z3 Z4 Z5) if _n <= 500 ,noconstant gridmin(0 `tl') gridmax(0 `tu') gridpoints(1 101) project(X1_weak_het) ptestlist (lc_2sls k_2sls)
