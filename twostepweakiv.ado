*! twostepweakiv 1.0.01 01Aug2017
*! authors Sun
* Usage:
* <eqn>, <options>         = estimate equation as model=linear
* Version notes:
* 1.0.01	01Aug2017.	First complete working versionm largely based on Finlay-Magnusson-Schaffer's weakiv
...
*** inexog and cons partialled out everywhere ***
		...
		//tempname first pi_z var_pi_z // looks like I cant declare pi_z and var_pi_z here as tempname because they are already tempname
		tempname first i
...
* LC weights a_min is calculated for the following range.
	...
	else if inlist(`level', 98,95,90) == 0 {
di as text "a_min is not pre-tabulated for this confidence level - need to simulate it and can take a while" // alpha_level not in a_min.csv
		local level = `level'
	}
	else {
		local level = `level'
...
di as text "a_min is not pre-tabulated for this confidence level - need to simulate it and can take a while" // gamma_level not in a_min.csv
...
)
{
		 ...
		 alpha_out = regexm(" 5 10 "," "+strofreal(a)+" ")
		 
		 if (k_df > 5 | nexexog > 50 | gamma_out == 0 | alpha_out == 0) {
		 //if (1<2) {
			printf("need to tabulate a_min...")
			...
			printf("v1 is %g",v[1,1])
			printf("v2 is %g",v[1,2])
		 ...
		 st_numscalar("r(lc_crit)",strtoreal(regexs(2)))
			printf("v1 is %g",strtoreal(regexs(1)))
			printf("v2 is %g",strtoreal(regexs(2)))
...
timer_on(1)
		...
		npd				=0										//  initialize
printf("here %g", cols(pi_z))
printf("here %g", cols(st_matrix(pi_z_name)))

printf("here %g", cols(st_matrix(zzinv_name)))
